    ...
    for (const r of reportArray) {
        ...
        if (r.hasOwnProperty("reportJson")) {
            ...
            if (r.reportJson.hasOwnProperty("fail")) {
                ...
                // Handle cases where the fail message is a JSON and not an
                // array
                if (r.reportJson.fail.constructor === Object) {
...
export const genericTableParser = (reportArray, action) => {
    ...
    for (const cell of reportArray) {
        ...
        dataDict[cell.rowId][accessor] =
            <CellBar clickPointer={action ? true : false} action={action ? (e) => {action(e, cell)} : () => {}} value={cell.value} max={columnMaxVals.get(cell.header)}/>;
